class MiddleCategoriesController < ApplicationController
  ...
  def index
    @middles = MiddleCategory.category_tree
    @middle_categories = @middles.group_by {|category| category.large_category_code}
  end
  ...
  private
    ...
    def middle_category_params
      params.require(:middle_category).permit(:large_category_code, :middle_category_code, :middle_category_name, :display_order)
    end
end
